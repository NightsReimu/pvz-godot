...
**Godot 4.6** · **当前版本 v1.0.110** · **Windows / macOS / Web / Android** · **横屏 1600x900 基准布局**
...
《植物大战僵尸 SVG 版》是使用 Godot 4.6 制作的横屏塔防同人项目。它以经典的格子种植、阳光管理和波次防守为核心，加入多世界主线、每日资源关、无尽三选一强化、植物养成、基建生产、抽卡与图鉴等系统，并制作了红魔乡、妖妖梦、永夜抄主题的东方 Boss 支线。
...
| 泳池时代 | 六行战场、水路、睡莲、游泳、冰道单位与永夜抄 Boss 支线 |
...
- 回旋镖经过火炬树桩会转为烈焰回旋镖：保留出回与回程轨迹，提升伤害并对命中点及邻近僵尸施加火焰溅射和持续灼烧。
...
泳池世界 3-19 至 3-21 依次迎战莉格露、米斯蒂娅和上白泽慧音。3-20 夜雀食堂和 3-21 月下的历史采用六行纯草地；同一角色先以道中非符拦路，击退后才开放终末。道中保留关卡音乐，小怪持续出现但波次与进度条冻结，终末登场时才切换专属音乐。
...
v1.0.103 新增 **3-22-b 雾雨魔理沙**，同样通过 3-21 解锁，共用灵梦的深夜竹林与道中 BGM，终末使用新提供的音乐与 24 帧透明动作。包含永夜抄 4B 四难度的 078–100 号符卡分支：彩色星带、旋转星流、非定向激光、单/双魔炮、地球光与射月，Normal 以上有终末魔炮。新增灼光格、棱镜格，以及可抢种阻止、可击破的魔化蘑菇。

露米娅、红美铃、咲夜、橙、爱丽丝、妖梦、莉格露、米斯蒂娅、慧音、灵梦、魔理沙的本关道中现已补齐。道中只有一段重复非符，血量为同难度终末总血量的 12%，必须被玩家击破，不按时间退场；援军持续出现，波次与进度条保持不动。击破后继续剩余波次，终末保留完整技能路线。该血量比例是本作的塔防平衡设定。

应战斗反馈，东方 Boss 数值适度回调：相对 v1.0.102，生命增加约 11%–15%、伤害增加约 15%–18%、弹速约增加 6%，密度与发射频率同步提高。25 种东方 Boss 统一人物显示高度，阶段不再放大角色。原作对应、塔防适配边界与数值表见 [东方 Boss 符卡对照](docs/touhou-spells.md)。
...
**v1.0.107 新增 3-23「秽土的高贵之笼」**，通过 3-22-a 与 3-22-b 后解锁。六行草地置于旋转的永远亭回廊，战斗格子保持稳定。道中因幡帝使用独立的低血量非符，击破后继续推进；铃仙终末切换新音乐，以永夜抄五面 101–119 号四难度分支展开 4 / 6 / 7 / 8 阶段。Normal 以上追加原创月门、月食与回廊折返符卡，并以「月兔远隔催眠术」耐久收束。

铃仙的幻视弹以空心轮廓显示且不判伤，恢复实弹时先预警；红眼格暂时扰乱植物攻速与瞄准。月门倒计时 4 秒后召唤月兔，可被攻击或抢先种植关闭。治愈葫芦固定加入传送带，只负责持续恢复植物生命；它不会免疫狂气、月食或月光冲击。新增月兔斥候、月兔卫兵，帝与铃仙各使用用户提供的 24 帧透明素材；其余东方人物素材保持。同期修正 11 个同角色道中的生命、击破门槛、小怪与波次冻结逻辑，以及战斗符名横幅遮挡卡槽的问题。
...
**v1.0.110 新增 3-24-a 八意永琳**：低血量道中、回廊出口到巨月天空、六组永夜抄 6A 原作符卡与高难度原创医术，四难度 6 / 7 / 8 / 9 阶段。临时水/岩浆/屋顶、可击碎的狂暴与禁疗药瓶、全世界普通敌群及星弹精灵/分裂毛玉/飞艇玉兔。东方葫芦相对出现权重降低 30%，只负责回血。[完整版本说明](docs/releases/v1.0.110.md)。

**v1.0.109 修正治愈葫芦职责**：治愈葫芦只负责持续回血，不再提供 3×3 保护、狂气免疫、月食射程保护或月光减伤；3-23 的铃仙和混合小怪压力调整保持。
...
v1.0.106 新增火炬树桩与回旋镖的组合机制。普通回旋镖和集群回旋镖穿过同一行的火炬树桩后会变成烈焰回旋镖，沿用原本的出回路径并获得橙红火焰拖尾；命中会造成约半径一格的火焰溅射，并给主目标和溅射目标施加持续灼烧。单个回旋镖只会被点燃一次，反射或回程不会重复增伤。实现与验证见 [烈焰回旋镖记录](docs/flame-boomerang.md)。
