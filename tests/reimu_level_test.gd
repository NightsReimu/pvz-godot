...
func _initialize() -> void:
	...
	check(stage.get("mid_boss_kind", "") == "reimu_boss" and stage.get("mid_boss_final_preview", false) and stage.unlock_requirements == ["3-21"], "Reimu previews her own finale before the final route")
	...
	for rank in range(4):
		...
		check(Spells.phases_for("reimu_boss", level).size() == ids[rank].size() + rank, "Preserve all canonical cards and cumulatively add one original move per tier")
