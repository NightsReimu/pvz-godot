...
func _test_selection_background_preview_styles_cover_level_terrains() -> bool:
	...
	if passed:
		var levels = [
			...
			{"id": "2-29-preview", "terrain": "spiral_staircase", "row_count": 5, "events": []},
		...
		for level_variant in levels:
			...
			if String(level.get("terrain", "")) == "spiral_staircase":
				passed = _assert_true(String(style.get("terrain_key", "")) == "spiral_staircase", "spiral_staircase preview should use its own stair terrain style instead of the generic fallback") and passed
				passed = _assert_true(String(game.call("_selection_preview_cell_kind", style, 2, 4)) == "stair", "spiral_staircase preview should render stair cells") and passed
