...
func _run() -> void:
	...
	failed = not _assert_true(String(level.get("mid_boss_kind", "")) == "mystia_boss" and bool(level.get("mid_boss_final_preview", false)), "3-20 should use Mystia's short original-style road appearance") or failed
