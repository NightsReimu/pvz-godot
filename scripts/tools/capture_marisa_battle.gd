...
const CAPTURE_PATH := "res://output/marisa"
...
func _run() -> void:
	...
	for viewport in [Vector2i(1600, 900), Vector2i(844, 390)]:
		...
		for choice in ["easy", "normal", "hard", "lunatic"]:
			...
			var cards: Array = game.TouhouSpellDefs.cards_for("marisa_boss", game.current_level)
			for cycle in range(cards.size()):
				...
				game.marisa_runtime.update(1.4)
...
func snapshot(game: Control, label: String) -> void:
	...
	print("Marisa capture %s: %s" % [label, error_string(capture.save_png(path))])
