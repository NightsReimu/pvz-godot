...
static func plant_lines(kind: String) -> Array:
	match kind:
		...
		"prism_grass":
			return ["棱镜束会穿透一整排多个目标。", "能量豆后会射出更长更重的贯穿光束。"]
		"lantern_bloom":
...
static func zombie_lines(kind: String) -> Array:
	match kind:
		...
		"dancing":
			return ["舞王进入舞台后会召唤伴舞。", "会在附近位置持续补足伴舞人数。"]
		"backup_dancer":
			return ["由舞王召唤的随从单位。", "单体不强，但会迅速占满前线空间。"]
		"farmer":
		...
		"nether":
			return ["冥界僵尸会优先从墓碑附近钻出。", "灯火会让周围植物短暂睡眠。"]
		"ducky_tube":
			return ["基础游泳僵尸，会在泳池水路里持续推进。", "本体不强，但会把水路压力常态化。"]
		"snorkel":
