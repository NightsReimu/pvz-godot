...
static func plant_lines(kind: String) -> Array:
	...
	match kind:
		...
		"origami_blossom":
			return ["魔术花每秒都会随机抖出一种已经存在的魔术弹，整路有目标就会不断变着花样开火。", "点击大招会朝整行甩出一大串随机魔术弹，能量豆时也会进入更密集的随机弹幕秀。"]
		"chimney_pepper":
			return ["烟囱椒炮会朝屋顶前方抛出爆裂火团，落地后还能顺带灼烧附近目标。", "能量豆会触发一轮短促但高压的火团连射。"]
		"tesla_tulip":
			return ["特斯拉郁金香会把链雷从本路前排一路串过去，串到的目标越多，整串雷击伤害越高。", "点击大招会召来一辆 Tesla Model Y 沿本路横扫，能量豆则会连续放出更猛的强化链雷。"]
		"brick_guard":
			return ["砖卫是屋顶延长战的厚重防线，适合扛住扶梯、炮塔和巨人的持续拆阵。", "点击大招会把所在整列一起垒成砖墙，能量豆会立即补满砖层并叠上一层更厚的护甲。"]
		"signal_ivy":
			return ["信号常春藤会周期性打出脉冲，顺带显形隐蔽与空中单位，还能替周围植物屏蔽程序员僵尸的降速干扰。", "能量豆会把信号塔扩成大范围高频扫描。"]
		"roof_vane":
			return ["风向草会在身前持续卷出圆形风击区，把闯进来的僵尸一边吹退一边刮伤。", "能量豆会掀起一阵更远更强的屋脊狂风。"]
		"skylight_melon":
		...
		"cotton_candy":
			return ["棉花糖只能放在云朵上。每 2.8 秒释放糖云，治疗周围 3×3 植物 28 点，并使范围内僵尸减速 35% 持续 2.6 秒。", "棉花糖庆典会立即治疗周围 3×3 植物 180 点，并绽放糖云高光。"]
		"mango_bowling":
		...
		"dragon_bubble_pult":
			return ["龙泡泡投手投出会分裂的龙泡泡，空中裂成三发分散砸向尸群。", "投掷类植物能越过火山坡面，是凹凸地形的主力输出。"]
		"cork_plug":
			return ["木塞子只能种在火山岩浆格上，塞住后该格不再喷火并可种植。", "想用地形格子时，先用木塞子把岩浆堵住。"]
		"cyclone_grass":
			return ["中控旋风草是一次性植物，使用后把周围 3×3 的僵尸全吸过来聚歼。", "适合在尸潮扎堆时一击清场。"]
		"sand_lotus":
			return ["沙尘莲会把该格的僵尸陷进泥沙定身，对空与 1600 血以上的单位无效。", "用来卡住冲家的高机动单位非常有效。"]
		"frost_boomerang":
			return ["寒冰回旋镖射手射出往返的寒冰回旋镖，往返都能伤害并减速僵尸。", "回旋镖能打中身后绕过来的敌人。"]
		"toxic_gum_pult":
			return ["剧毒口香糖投手投出的口香糖会分裂到附近僵尸，命中有概率定身。", "面对密集尸群时，定身能直接打断一波推进。"]
		"corn_cannon":
			return ["玉米加农炮右键点击可发射到指定位置造成范围爆炸。", "左键充能满后会触发饱和炮击，是火山世界最重型的火力。"]
		"holy_flower":
			return ["圣光花种在别的植物上面，会持续给宿主植物提供护盾。", "把它套在主力输出上，能让它们扛住岩浆喷火与冲锋。"]
		"ice_cream":
			return ["冰淇淋种在别的植物上面，一次性让该植物的大招瞬间冷却完成。", "需要立刻爆发时的关键加速道具。"]
		"gator_cannon":
			return ["鳄鱼炮向前发射穿透的激光龙珠，能贯穿整路僵尸。", "面对成排推进时，一发就能打出穿透伤害。"]
		_:
...
static func zombie_lines(kind: String) -> Array:
	...
	match kind:
		...
		"city_boss":
			return ["霓虹尸王是城市世界的最终 Boss，会把主线出现过的非 Boss 僵尸一口气混进同一场暴风雪终章。", "它会反复召来整城尸潮、封锁街区格位，并用霓虹冲击波持续压缩你的阵型。"]
		"rumia_boss":
			return ["露米娅是操纵黑暗的妖怪，盘旋在血月支线右侧。", "夜符「Night Bird」以环形弹配合自机狙，暗符「Demarcation」交错展开红蓝弹幕。"]
		"daiyousei_boss":
			return ["大妖精是寒湖支线的中 Boss，在右侧释放定向散弹。", "她在《红魔乡》没有命名符卡；击退她后关卡继续推进。"]
		"cirno_boss":
			return ["琪露诺是操纵冷气的冰之妖精，在寒湖使用冰柱散落、完美冻结和钻石风暴；雪夜道中采用《妖妖梦》Normal 非符攻击。", "完美冻结会让已经发出的冰弹停止，短暂停顿后改变方向继续飞行。"]
		"meiling_boss":
			return ["红美铃是操纵气的红魔馆门番。", "芳华绚烂、彩虹的风铃、彩雨与极彩台风依次展开花瓣状、彩色环形及旋转弹幕。"]
		"koakuma_boss":
			return ["小恶魔是血色图书馆的中 Boss，在书架前展开红色环弹和定向蓝弹。", "她在《红魔乡》没有命名符卡；击退她后才能继续接近帕秋莉。"]
		"patchouli_boss":
			return ["帕秋莉使用火、水、木、金、土、日、月属性魔法；木属性不等于额外的风属性。", "图书馆采用《红魔乡》灵梦灵符路线；芙兰关卡中使用 Extra 的沉静的月神、皇家烈焰与贤者之石。"]
		"sakuya_boss":
			return ["十六夜咲夜操纵时间，以飞刀和时停构成弹幕。钟表的残骸、月神之钟会停住弹幕并布置新的飞刀，时间恢复后飞刀继续飞行。", "终幕使用误导、钟表的残骸、月神之钟、操弄玩偶；作为下一关中 Boss 时使用 Eternal Meek。"]
		"remilia_boss":
			return ["蕾米莉亚是红魔馆的吸血鬼之主，拥有操纵命运程度的能力。", "大卫之星、红色的冥界、弗拉德·特佩斯的诅咒、绯红之击、红魔法组成《红魔乡》Normal 符卡序列。猩红长夜的持续血线压力属于本关规则。"]
		"letty_boss":
			return ["蕾蒂是操纵寒气的冬之妖怪。", "《妖妖梦》Normal 的寒符「Lingering Cold」与冬符「Flower Wither Away」交替释放寒色环弹与散弹。"]
		"chen_boss":
			return ["橙是八云蓝的式神，使用妖术，在高速运动的轨迹上展开弹幕。", "通常使用凤凰卵、飞翔晴明、天仙鸣动与尸解永远；八云支线中 Boss 使用青鬼赤鬼、飞翔毘沙门天。"]
		"alice_doll_zombie":
			return ["爱丽丝人偶是爱丽丝召唤出的 Boss 专属敌人，不会出现在主线事件表。", "人偶体型小但推进稳定，常和牵线弹幕、墓碑升起一起扰乱防线。"]
		"alice_boss":
			return ["爱丽丝操纵人偶，以分散的人偶阵位施放魔法。", "法兰西、荷兰、伦敦与上海人偶依次展开定向弹、环弹、交错弹与有预警的激光。森林的墓碑是关卡地形。"]
		"lily_white_boss":
			return ["莉莉霍瓦特是告知春天到来的妖精，出现在云海中途。", "她在《妖妖梦》没有命名符卡，以密集散弹和环弹报春。"]
		"prismriver_boss":
			return ["骚灵三姐妹演奏无需接触的乐器，小提琴、号与键盘各自发出不同轨迹的音乐弹幕。", "本关采用露娜萨独奏路线，包含幽灵絮语、Guarneri del Gesù、棱镜协奏曲和灵车大协奏曲。"]
		"youmu_boss":
			return ["魂魄妖梦是半人半灵的剑士，以楼观剑、白楼剑及半灵构成剑气弹幕。", "保留原创符卡魂魄「怨灵使役」：可被攻击的怨灵接触植物后施加短暂魅惑，令其停止正常行动并攻击附近同伴，随后恢复。"]
		"youmu_wraith":
			return ["妖梦怨灵是魂魄妖梦召出的半灵残影，会沿行高速冲向最近的植物。", "它可以被攻击消灭；若撞上植物，会造成轻伤并短暂魅惑目标，让植物攻击附近同伴。"]
		"yuyuko_spirit":
			return ["幽幽子亡灵会从墓碑与西行妖樱花中爬出，沿行追向最近植物。", "它是可被攻击的实体；命中植物后会造成轻伤并短暂魅惑目标，随后化作樱花散去。"]
		"yuyuko_boss":
			return ["西行寺幽幽子是拥有诱导死亡能力的亡灵公主，以亡灵、蝶与樱花施放符卡。", "完全墨染的樱花之后进入「反魂蝶 -三分咲-」耐久终幕，倒计时结束后退场。西行妖封印没有被成功解除，这并非真正复活。"]
		"ran_boss":
			return ["八云蓝是八云紫的九尾狐式神，也能使役自己的式神橙。", "本关采用完整 Extra 十符序列，式神「橙」由橙的出弹点协同攻击；击退蓝后接战紫是本作的连续 Boss 规则。"]
		"yukari_boss":
			return ["八云紫是操纵境界的隙间妖怪。", "本关采用 Phantasm 十一符序列，包括动与静的均衡、光明与黑暗的网目、二重黑死蝶、式神「八云蓝」，最终展开紫奥义「弹幕结界」。"]
		"flandre_boss":
			return ["芙兰朵露拥有破坏事物程度的能力，以《红魔乡》Extra 十张符卡迎战。", "四重存在由本体与三个分身共同出弹，折反射会在边界反弹；之后就一个人都没有了吗为耐久攻击，最终以 QED「495年的波纹」加密环弹。"]
		"ninja":
		...
		"umbrella_zombie":
			return ["雨伞僵尸举起雨伞，能挡下一切投掷类攻击。", "盾牌约等于一个路障的耐久，用直射类植物拆掉它的伞最快。"]
		"shania_zombie":
			return ["傻妞僵尸飞行于空中，不会被地面植物近战啃咬。", "它会周期性操纵附近僵尸加速冲家，是必须优先用对空火力解决的威胁。"]
		"shade_zombie":
			return ["幽影僵尸会向前瞬移越过防线，最多瞬移两次。", "每次瞬移有几率变成任意传送，减速类植物能压住它的节奏。"]
		"crab_zombie":
			return ["螃蟹僵尸死后会变成一只幼蟹高速冲锋。", "别以为打掉本体就结束了，幼蟹跑得比本体快得多。"]
		"crabling":
			return ["幼蟹是螃蟹僵尸死后留下的高速冲锋单位。", "血量很低，但速度极快，很容易漏防冲进家里。"]
		"camel_zombie":
			return ["骆驼僵尸以三只一排同步推进，像一座移动的沙墙。", "它们共享压力，必须同时压制整排才能瓦解推进。"]
		"volcano_boss":
